...
function getNotice(notice) {
	...
	noticeDetailDescription[0].innerHTML = "작성지" + notice.userId;
	noticeDetailDescription[1].innerHTML = "작성일" + notice.createDate;
	noticeDetailDescription[2].innerHTML = "조회수" + notice.noticeCount;
	
	noticeContent.innHTML = "<h3> 첨부파일 </h3>";
	
	let noticeFileArray = new Array();
	notice.downloadFiles.forEacah(file => {
		if(file.fileCode != undefined) {
			noticeFileArray.push(`<a hred ="/api/v1/notice/file/download/${file.fileTempName}">${file.fileOriginName}</a>`)
		}
	})
